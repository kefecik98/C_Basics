...
distance (const DataPoint * datapoint, const Centroid * centroid)
{
  ...
  int dims = datapoint -> dimension;
  int *dataP = datapoint -> data;
  int *dataC = centroid -> data;

  // find Euclidean distance and then return 'sum' without calling sqrt
  for (int i = 0; i < dims; i++)
    { 
      sum += pow((dataP[i] - dataC[i]), 2);
    }
...
int closestCentroid (int kval, DataPoint * datapoint, Centroid * *centroids)
{
  ...
  minDist = distance(datapoint, centroids[0]);
  mindex = 0;
  
  for (int index = 1; index < kval; index++)
    {
      if (distance(datapoint, centroids[index]) < minDist)
	{
	  minDist = distance(datapoint, centroids[index]);
	  mindex = index;
	}
    }
...
void kmean (int kval, int nval, DataPoint * *datapoints, Centroid * *centroids)
{
  ...
  for (int i = 0; i < nval; i++)
    {
      centNo = rand() % kval;
      datapoints[i] -> cluster = centNo;
      Centroid_addPoint(centroids[centNo], datapoints[i]);
    }
  ...
  while (convergence != 1)
    {      
      ...
      for (int i = 0; i < nval; i++)
	{
	  if (datapoints[i] -> cluster != closestCentroid(kval, datapoints[i], centroids))
	    {
