...
int main(int argc, char **argv)
{
	...
	if (argc < 3){
		printf("need at least 3 input arguments");
		return EXIT_FAILURE;
	}
	// The program should then create 2 binary trees from input arguments
	treeNode * root1 = BinaryTreeCreate(argv[1]);
	treeNode * root2 = BinaryTreeCreate(argv[2]);
	...
	if ((root1 == NULL) || (root2 == NULL)){
		PrintTn1ContainedTn2(false, false);
		return EXIT_SUCCESS;
	}
	// 1. First check if `tn1` is contained in `tn2`.
	bool tn1_contained_tn2 = isContained(root2, root1);
	// 2. Second you should check if tree2 is contained in tree1,
	bool tn2_contained_tn1 = isContained(root1, root2);
